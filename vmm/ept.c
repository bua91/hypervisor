...
ept_pml4e_walk(epte_t *eptrt, const void *gpa, int create, epte_t **epte_out)
{
        ...
        if (pdpt_base == NULL) {
                ...
                return ept_pdpe_walk(KADDR((uint64_t)pdpt_base), gpa, create, epte_out);
...
int ept_pdpe_walk(epte_t *pdpt_base,const void *gpa,int create, epte_t **epte_out)
{
        ...
        if (pgdir_base == NULL)
        {
                ...
                return ept_pgdir_walk(KADDR((uint64_t)pgdir_base), gpa, create, epte_out); 
}
